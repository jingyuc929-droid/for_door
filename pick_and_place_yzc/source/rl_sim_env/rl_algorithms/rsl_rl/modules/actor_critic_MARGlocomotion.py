...
class ActorCriticMARGlocomotion(nn.Module):
    def __init__(
        self,
        module_cfg_dict
    ):
    ...
    def act(self, obs):
        ...
        gt_heightmap_obs = self.get_heightmap_obs(obs)
        heightmap_out = self.heightmap_encoder(gt_heightmap_obs)
        if 'estimator_net_out' in obs:
            obs = torch.cat((obs['estimator_net_out'], actor_obs, heightmap_out), dim=-1)
        else:
            obs = torch.cat((actor_obs, heightmap_out), dim=-1)
    ...
    def evaluate(self, obs):
        ...
        gt_heightmap_obs = self.get_heightmap_obs(obs)
        heightmap_out = self.heightmap_encoder(gt_heightmap_obs)
        obs = torch.cat((privileged_obs, heightmap_out, critic_obs), dim=-1)
        # print(f"critic_obs: {critic_obs}")
        # print(f"gt_heightmap_obs: {gt_heightmap_obs}")
