...
class AMPVAEVITOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cuda:0"):
        ...
        num_heightmap_obs_h = config_summary_env.num_heightmap_obs_h
        num_heightmap_obs_w = config_summary_env.num_heightmap_obs_w
        partial_hmap_obs_his = config_summary_env.partial_hmap_obs_his
        partial_hmap_obs_num = num_heightmap_obs_h * num_heightmap_obs_w * partial_hmap_obs_his
        num_heightmap_obs = num_heightmap_obs_h * num_heightmap_obs_w
        footheight_latent_out_dim = config_summary_env.footheight_latent_out_dim
        obs_latent_out_dim = config_summary_env.obs_latent_out_dim
        vel_out_dim = config_summary_env.vel_out_dim
        num_footheight_obs = config_summary_env.num_footheight_obs
        num_amp_obs = config_summary_env.num_amp_obs
        prop_obs_dim = config_summary_env.prop_obs_dim
        prop_obs_his = config_summary_env.prop_obs_his
        vel_out_dim = config_summary_env.vel_out_dim
        num_footheight_obs = config_summary_env.num_footheight_obs
        heightmap_latent_out_dim = config_summary_env.heightmap_latent_out_dim

        cenet_out_dim = vel_out_dim + obs_latent_out_dim + footheight_latent_out_dim + heightmap_latent_out_dim
        num_next_obs = config_summary_env.num_next_obs
        num_actions = self.env.num_actions

        # evaluate the policy class
        # ActorCritic
        print("body_names:", self.env.unwrapped.scene["robot"].data.body_names)
        print("joint_names:", self.env.unwrapped.scene["robot"].data.joint_names)
        dof_range = (
        ...
        actor_critic_vit = ActorCriticVit(
            num_actor_obs=num_actor_obs + cenet_out_dim,
            num_critic_obs=num_critic_obs + num_heightmap_obs + num_footheight_obs,
        ...
        # VAE
        vae_vit: VAEVit = VAEVit(
            env_num=self.env.num_envs,
            point_history_in_dim=partial_hmap_obs_his,
            prop_obs_in_dim=prop_obs_dim * prop_obs_his,
            prop_decoder_out_dim=num_next_obs,
            heightmap_decoder_out_h_dim=num_heightmap_obs_h,
            heightmap_decoder_out_w_dim=num_heightmap_obs_w,
            footheight_decoder_out_dim=num_footheight_obs,
            heightmap_latent_out_dim=heightmap_latent_out_dim,
            footheight_latent_out_dim=footheight_latent_out_dim,
            obs_latent_out_dim=obs_latent_out_dim,
            vel_out_dim=vel_out_dim,
        ).to(self.device)

        # initialize algorithm
        self.alg = AMPVAEVITPPO(
            actor_critic_vit=actor_critic_vit,
        ...
        self.alg.init_storage(
            ...
            amp_obs_shape=[num_amp_obs],
            prop_obs_shape=[prop_obs_dim * prop_obs_his],
            lidar_obs_shape=[partial_hmap_obs_num],
            gt_vel_shape=[vel_out_dim],
            gt_footheight_shape=[num_footheight_obs],
            gt_heightmap_shape=[num_heightmap_obs],
            next_obs_shape=[num_next_obs],
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False, profiler=None):  # noqa: C901
        ...
        prop_his_obs = obs_dict["prop_obs"]
        point_his_obs = obs_dict["lidar_obs"]
        gt_vel = obs_dict["gt_vel_obs"]
        gt_footheight = obs_dict["gt_foot_scan_obs"]
        gt_heightmap = obs_dict["gt_heightmap_obs"]
        next_amp_obs = amp_obs.clone()
        actor_obs, critic_obs, amp_obs, prop_his_obs, point_his_obs, gt_vel, gt_footheight, gt_heightmap = (
            actor_obs.to(self.device),
            critic_obs.to(self.device),
            amp_obs.to(self.device),
            prop_his_obs.to(self.device),
            point_his_obs.to(self.device),
            gt_vel.to(self.device),
            gt_footheight.to(self.device),
            gt_heightmap.to(self.device),
        )
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.no_grad():
                ...
                for _ in range(self.num_steps_per_env):
                    # Sample actions
                    actions = self.alg.act(actor_obs, critic_obs, amp_obs, prop_his_obs, point_his_obs, gt_vel, gt_footheight, gt_heightmap)
                    if self.use_amp:
                    ...
                    # priv_obs = obs_buf["privileged_obs"]
                    next_amp_obs = obs_buf["amp_obs"]
                    prop_his_obs = obs_buf["prop_obs"]
                    point_his_obs = obs_buf["lidar_obs"]
                    gt_vel = obs_buf["gt_vel_obs"]
                    # gt_mass = obs_buf["gt_mass_obs"]
                    gt_footheight = obs_buf["gt_foot_scan_obs"]
                    gt_heightmap = obs_buf["gt_heightmap_obs"]
                    # print("critic_obs", critic_obs)
                    # print("actor_obs", actor_obs)
                    # print("amp_obs", amp_obs)
                    # print("vae_obs", vae_obs)
                    # Move to device
                    actor_obs, critic_obs, next_amp_obs, prop_his_obs, point_his_obs, gt_vel, gt_footheight, gt_heightmap, rewards, dones = (
                        actor_obs.to(self.device),
                        critic_obs.to(self.device),
                    ...
                    next_actor_obs = torch.clone(critic_obs.detach()[:, 0:33])
                ...
                # compute returns
                adaboot_p = self.alg.compute_returns(critic_obs, gt_footheight, gt_heightmap)
    ...
    def save(self, path: str, infos=None):
        ...
        if self.use_amp:
            saved_dict = {
                ...
                "vae_state_dict": self.alg.vae_vit.state_dict(),
    ...
    def load(self, path: str, load_optimizer: bool = True):
        ...
        self.alg.vae_vit.load_state_dict(loaded_dict["vae_state_dict"], strict=True)
    ...
    def get_inference_policy(self, device=None):
        ...
        if device is not None:
            ...
            self.alg.vae_vit.to(device)
        policy = self.alg.actor_critic
        vae = self.alg.vae_vit
        return policy, vae
