...
def foot_clearance(
    ...
    参考 :func:`foot_scan` 的射线投射逻辑，但仅保留每条腿 3×3 网格中的中间射线，
    ...
    if center_index is None:
        center_index = grid.shape[1] // 2

    if not (0 <= center_index < grid.shape[1]):
        raise ValueError(
            f"center_index={center_index} 超出射线数量范围 (0, {grid.shape[1] - 1})。"
        )
...
def random_com_obs(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    asset: RigidObject = env.scene[asset_cfg.name]
...
def random_mass_obs(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    asset: RigidObject = env.scene[asset_cfg.name]
...
def random_material_obs(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    asset: RigidObject = env.scene[asset_cfg.name]
...
def randomize_actuator_gains_obs(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg, kp_default: float, kd_default: float) -> torch.Tensor:
    ...
    kp = asset.actuators["base_legs"].stiffness.clone() - kp_default
    kd = asset.actuators["base_legs"].damping.clone() - kd_default
...
def randomize_actuator_lag_obs(env: ManagerBasedEnv) -> torch.Tensor:
    lag = env.scene.articulations["robot"].actuators["base_legs"].positions_delay_buffer.time_lags.clone()
    lag = lag.float().to(env.device)
    # print("lag", lag.shape)
...
def ee_external_forces_applied(env: ManagerBasedEnv) -> torch.Tensor:
    """末端外力（世界坐标系 3D）：Fx,Fy,Fz。

    由通用事件 `apply_external_force_torque_3d` 写入 `env.event_apply_forces_torques_buf`（wrench: Fx,Fy,Fz,Tx,Ty,Tz）。
    这里返回其前三维（Fx,Fy,Fz）。若事件未启用，则返回全零。
    """
    wrench = external_forces_torques_applied(env)  # (E, 6)
    return wrench[:, :3]
