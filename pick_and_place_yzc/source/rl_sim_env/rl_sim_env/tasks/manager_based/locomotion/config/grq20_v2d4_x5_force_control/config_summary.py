...
class ConfigSummary:
    ...
    class event:
        ...
        # 触发间隔（秒）：按需调大/调小
        ee_force_interval_range_s = (10.0, 10.0)
    ...
    class reward:   
        ...
        config_dict = {
            ...
            # 机械臂末端跟随（投影COM参考系下的 9D 目标点）+ 外力偏移
            # 说明：
            # - 三个点（main/x/y）共享同一个偏移 delta_p（由管理器统一计算与缓存）
            # - 偏移管理器参数在 ConfigSummary.command 中配置
            ...
            'track_ee_target_x_offset_exp_force_compliance': {
                'weight': 0.6,
                'params': {
                    'command_name': 'ee_target_points',
                    'ee_asset_cfg': SceneEntityCfg("robot", body_names=ROBOT_EE_BODY_NAME),
            ...
            'track_ee_target_y_offset_exp_force_compliance': {
                'weight': 0.6,
                'params': {
                    'command_name': 'ee_target_points',
                    'ee_asset_cfg': SceneEntityCfg("robot", body_names=ROBOT_EE_BODY_NAME),
