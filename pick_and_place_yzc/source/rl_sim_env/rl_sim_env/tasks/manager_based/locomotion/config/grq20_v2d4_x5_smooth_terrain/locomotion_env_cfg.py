...
@configclass
class LocomotionVaeEnvCfg(LocomotionEnvCfg):
    def __post_init__(self):
        ...
        # terrain settings:
        # 说明：
        # - 仅降低 num_rows 只是减少课程档位，不会改变“每档内部”的最大地形强度。
        # - 这里对 smooth_terrain 任务做独立地形软化：直接降低关键地形参数上限。
        self.scene.terrain = LOCOMOTION_TERRAIN_CFG2d4.copy()
        if self.scene.terrain.terrain_generator is not None:
            tg = self.scene.terrain.terrain_generator
            # 保留课程学习，但仅 0/1/2 三个等级
            tg.num_rows = 3
            # 显式设置各类地形占比（总和=1.0），确保 rough/离散地形稳定出现
            if "pyramid_stairs" in tg.sub_terrains:
                tg.sub_terrains["pyramid_stairs"].proportion = 0.14
            if "pyramid_stairs_inv" in tg.sub_terrains:
                tg.sub_terrains["pyramid_stairs_inv"].proportion = 0.14
            if "hf_pyramid_slope" in tg.sub_terrains:
                tg.sub_terrains["hf_pyramid_slope"].proportion = 0.17
            if "hf_pyramid_slope_inv" in tg.sub_terrains:
                tg.sub_terrains["hf_pyramid_slope_inv"].proportion = 0.17
            if "plane_run" in tg.sub_terrains:
                tg.sub_terrains["plane_run"].proportion = 0.04
            if "boxes" in tg.sub_terrains:
                tg.sub_terrains["boxes"].proportion = 0.12
            if "random_rough" in tg.sub_terrains:
                tg.sub_terrains["random_rough"].proportion = 0.22
            # 降低台阶/坡度/粗糙高度上限，获得更“顺滑”的地形
            if "pyramid_stairs" in tg.sub_terrains:
                tg.sub_terrains["pyramid_stairs"].step_height_range = (0.01, 0.20)
            if "pyramid_stairs_inv" in tg.sub_terrains:
                tg.sub_terrains["pyramid_stairs_inv"].step_height_range = (0.01, 0.20)
            if "boxes" in tg.sub_terrains:
                tg.sub_terrains["boxes"].grid_height_range = (0.01, 0.08)
            if "random_rough" in tg.sub_terrains:
                tg.sub_terrains["random_rough"].noise_range = (0.01, 0.08)
                tg.sub_terrains["random_rough"].noise_step = 0.02
            # 斜坡强度上调约 20%: 0.18 -> 0.216
            if "hf_pyramid_slope" in tg.sub_terrains:
                tg.sub_terrains["hf_pyramid_slope"].slope_range = (0.0, 0.40)
            if "hf_pyramid_slope_inv" in tg.sub_terrains:
                tg.sub_terrains["hf_pyramid_slope_inv"].slope_range = (0.0, 0.40)
        # 初始地形等级上限同样限制到 2
        ...
        self.scene.fl_foot_scanner = FOOT_SCANNER_CFG.copy()
        ...
        self.scene.fr_foot_scanner = FOOT_SCANNER_CFG.copy()
        ...
        self.scene.rl_foot_scanner = FOOT_SCANNER_CFG.copy()
        ...
        self.scene.rr_foot_scanner = FOOT_SCANNER_CFG.copy()
        ...
        terrain_keys = list(sub_terrains.keys())
        proportions = [float(sub_terrains[key].proportion) for key in terrain_keys]
        total_prop = sum(proportions)
        if total_prop <= 0.0:
            raise ValueError("Terrain proportion sum must be positive for command group allocation.")

        # 按比例归一化分配 env，保证总数严格等于 num_envs（避免 int 截断导致缺口）
        normalized = [p / total_prop for p in proportions]
        expected_counts = [w * num_envs for w in normalized]
        base_counts = [int(x) for x in expected_counts]
        remaining = num_envs - sum(base_counts)
        # 余数按小数部分从大到小补齐
        frac_order = sorted(
            range(len(terrain_keys)),
            key=lambda i: expected_counts[i] - base_counts[i],
            reverse=True,
        )
        ...
        env_start = 0
        for idx, key in enumerate(terrain_keys):
        ...
        self.commands.ee_target_points = create_projected_com_target_points_command_cfg(
            ...
            ground_z_lpf_alpha=self.config_summary.command.ee_target_ground_z_lpf_alpha,
            use_projected_origin=True, 
        )
        ...
        # startup event: disable collisions for arm links to prevent arm-ground contacts.
        # This is task-specific and does not affect other locomotion configs.
        self.events.disable_arm_collision = EventTerm(
            func=mdp.set_body_collision_enabled,
            mode="startup",
            params={
                "body_names": ["link1", "link2", "link3", "link4", "link5", "link6"],
...
@configclass
class LocomotionVaeEnvCfg_PLAY(LocomotionVaeEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
class LocomotionVaeEnvCfg_ONNX_PLAY(LocomotionVaeEnvCfg_PLAY):
    def __post_init__(self):
        super().__post_init__()
...
@configclass
class LocomotionVaeEnvCfg_DEBUG_PLAY(LocomotionVaeEnvCfg_PLAY):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
class LocomotionVaeEnvCfg_REPLAY_AMPDATA(LocomotionVaeEnvCfg_PLAY):
    def __post_init__(self):
        super().__post_init__()
