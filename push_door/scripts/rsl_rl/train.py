...
_PUSH_CONSOLE_METRIC_ORDER = (
    ...
    "stage_gated_reward/ee_to_handle",
    "stage_gated_reward/ee_to_target_shaped",
    "stage_gated_reward/approach_progress",
    "stage_gated_reward/ee_handle_contact",
    "stage_gated_reward/ee_to_handle_shaped",
    "stage_gated_reward/align_hook_near_handle",
    "stage_gated_reward/ee_above_handle_target",
    ...
    "--------------transition-----------",
    "stage_gated_reward/grasp_success_transition",
...
_PULL_CONSOLE_METRIC_ORDER = (
    "Episode_Termination/time_out",
    "Episode_Termination/base_traverse_success",
    "Episode_Termination/base_fall",
    "Episode_Termination/base_out_of_hinge_radius",
    "Episode_Termination/bad_arm_pose",
    "--------------stage 0--------------",
    "stage_gated_reward/base_to_pick_stance",
    "stage_gated_reward/ee_to_handle",
    "stage_gated_reward/ee_to_target_shaped",
    ...
    "stage_gated_reward/stall_after_press",
    "--------------stage 2 pull--------------",
    "stage_gated_reward/pull_door",
    "--------------release--------------",
    "base_reward/release_handle_after_open",
    "--------------traverse--------------",
    "base_reward/pull_follow",
    "base_reward/pull_traverse",
    "base_reward/pull_traverse_reached_activation",
    "base_reward/pull_traverse_active",
    "base_reward/pull_traverse_state",
    "base_reward/pull_traverse_z1",
    "base_reward/pull_traverse_z2",
    "base_reward/pull_traverse_z3",
    "base_reward/pull_traverse_entered_z1",
    "base_reward/pull_traverse_entered_z2",
    "base_reward/pull_traverse_illegal_z2",
    "base_reward/traverse_success_bonus",
    "base_reward/success_done",
    "base_reward/success_signed_forward",
    "--------------safety--------------",
    "base_reward/safety",
...
_PULL_CONSOLE_DISPLAY_ALIASES = {
    "stage_gated_reward/push_door": "stage_gated_reward/pull_door",
}
...
def _should_print_console_metric(metric_name: str, metric_order: tuple[str, ...]) -> bool:
    return metric_name in metric_order


def _ordered_console_metric_lines(
    metric_lines: dict[str, str],
    metric_order: tuple[str, ...],
    display_aliases: dict[str, str],
) -> list[str]:
    """Build the configured metric block, including human-readable group separators."""
    if not metric_lines:
        return []

    first_line = next(iter(metric_lines.values()))
    ...
    for name in metric_order:
        if _is_console_separator(name):
            pending_separator = f"{indent}{name}"
            continue
        ...
        if alias is not None:
            line = line.replace(name, alias, 1)
        output.append(line)

    # Keep any whitelisted metrics that were not listed explicitly robust to
    # future additions, without changing the configured block above.
    configured = set(metric_order)
    output.extend(line for name, line in metric_lines.items() if name not in configured)
    return output


def _filter_console_log_text(
    text: str,
    mode: str,
    metric_order: tuple[str, ...],
    display_aliases: dict[str, str],
    ...
    for line in lines:
        ...
        if mode == "stage_and_termination" and _should_print_console_metric(metric_name, metric_order):
            metric_lines.setdefault(metric_name, line)

    if mode == "minimal" or len(metric_lines) == 0:
        return "\n".join(kept_lines) + ("\n" if text.endswith("\n") else "")

    ordered_metric_lines = _ordered_console_metric_lines(metric_lines, metric_order, display_aliases)
    ...
    def filtered_log(self, *args, **kwargs):
        ...
        def filtered_print(*print_args, **print_kwargs):
            if len(print_args) == 1 and isinstance(print_args[0], str):
                print_args = (_filter_console_log_text(print_args[0], mode, metric_order, display_aliases),)
            return original_print(*print_args, **print_kwargs)

        def filtered_logger_info(logger_self, msg, *info_args, **info_kwargs):
            if isinstance(msg, str):
                msg = _filter_console_log_text(msg, mode, metric_order, display_aliases)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if not args_cli.headless:
        try:
            ...
            handle_target_offset_h = (-0.08, 0.04, -0.005)
    ...
    if False and not args_cli.headless:
        try:
            ...
            VIS_HANDLE_OFFSET_H = (-0.08, 0.04, -0.005)
   ...
   # create runner from rsl-rl
    ...
    console_metric_order = (
        _PULL_CONSOLE_METRIC_ORDER if is_pull_task else _PUSH_CONSOLE_METRIC_ORDER
    )
    console_display_aliases = _PULL_CONSOLE_DISPLAY_ALIASES if is_pull_task else {}
    print(f"[INFO] Console metric profile: {'pull' if is_pull_task else 'push'}")
    runner = _patch_runner_console_log(
        runner,
        console_log_mode,
        console_metric_order,
        console_display_aliases,
    )
