...
PRIVILEGED_STATE_SCHEMA = (
    ...
    ("door_open_direction", 1),
)

PULL_PRIVILEGED_STATE_SCHEMA = PRIVILEGED_STATE_SCHEMA + (
    ("zone_one_hot", 4),
    ("traverse_state_one_hot", 4),
    ("traverse_started", 1),
    ("traverse_cheat_given", 1),
)
...
def export_doorbot_teacher(policy, path: str, jit_filename="policy.pt", onnx_filename="policy.onnx"):
    ...
    privileged_dim = policy.privileged_obs_dim
...
class DoorBotPrivilegedActorCritic(ActorCritic):
    ...
    def __init__(
        ...
        self.privileged_obs_dim = int(priv_dim)
...
class DoorBotDistillationVecEnvWrapper(RslRlVecEnvWrapper):
    ...
    def _door_angle(self):
        env_cfg = self._base_env().cfg
        mechanism = getattr(getattr(env_cfg, "events", None), "door_mechanism", None)
        params = getattr(mechanism, "params", None)
        door_open_sign = float(params.get("door_open_sign", 1.0)) if isinstance(params, dict) else 1.0
        return torch.clamp(door_open_sign * self._door_joints()[2], min=0.0)
    ...
    def step(self, actions):
        ...
        if self.collect_distillation_rollout:
            ...
            hpos, hvel, dpos, dvel = self._door_joints()
            ...
            extras["distillation"] = {
                ...
                "history_write_index": torch.full(
                    (self.num_envs,), self._history_write_index, device=self.device, dtype=torch.long
                ),
...
class DoorBotTeacherRunner(OnPolicyRunner):
    ...
    def save(self, path, infos=None):
        ...
        mechanism = getattr(getattr(base_env.cfg, "events", None), "door_mechanism", None)
        mechanism_params = getattr(mechanism, "params", None)
        door_open_sign = (
            float(mechanism_params.get("door_open_sign", 1.0))
            if isinstance(mechanism_params, dict)
            else 1.0
        )
        ...
        checkpoint.update({
            ...
            "privileged_state_schema_and_order": (
                PULL_PRIVILEGED_STATE_SCHEMA
                if policy.privileged_obs_dim == sum(dim for _, dim in PULL_PRIVILEGED_STATE_SCHEMA)
                else PRIVILEGED_STATE_SCHEMA
            ),
            "z_priv_dim": policy.z_priv_dim,
            "action_scale_and_clip": self._action_metadata(),
            "fixed_body_height": float(self.env.unwrapped.action_manager._terms["high_level_action"].cfg.default_body_height),
            ...
            "door_task_metadata": {
                "task_id": getattr(env_spec, "id", None),
                "door_mode": "pull" if door_open_sign < 0.0 else "push",
                "door_open_sign": door_open_sign,
            },
        })
