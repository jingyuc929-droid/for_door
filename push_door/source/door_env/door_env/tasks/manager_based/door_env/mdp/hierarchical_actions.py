...
DEFAULT_RL_ALGORITHMS_SOURCE_ROOT = os.path.abspath(
    os.environ.get(
        "DOORBOT_LOW_LEVEL_RL_SOURCE_ROOT",
        "/home/jing/pick_and_place_yzc/source/rl_sim_env",
    )
)
...
def _load_low_level_policy_classes(rl_source_root: str):
    ...
    if not os.path.isfile(actor_path) or not os.path.isfile(vae_path):
        raise ModuleNotFoundError(
            ...
            "Set DOORBOT_LOW_LEVEL_RL_SOURCE_ROOT to the relocated source/rl_sim_env directory."
        )
...
class HighLevelDoorOpenAction(ActionTerm):
    ...
    def __init__(self, cfg: "HighLevelDoorOpenActionCfg", env: ManagerBasedEnv):
        ...
        self._stage2_qerr_soft_limit = _as_tensor(
            cfg.stage2_qerr_soft_limit, self.device, self._num_joints, "stage2_qerr_soft_limit"
        ).view(1, -1)
        self._stage2_qerr_hard_limit = _as_tensor(
            cfg.stage2_qerr_hard_limit, self.device, self._num_joints, "stage2_qerr_hard_limit"
        ).view(1, -1)
        if torch.any(self._stage2_qerr_hard_limit <= self._stage2_qerr_soft_limit):
            raise ValueError("stage2_qerr_hard_limit must be greater than stage2_qerr_soft_limit")
    ...
    def _sync_targets_on_unlock_transition_if_needed(self) -> None:
        ...
        sync_mask = newly_unlocked if self.cfg.sync_arm_target_on_unlock else torch.zeros_like(newly_unlocked)
        if torch.any(sync_mask):
            q = self._asset.data.joint_pos[:, self._joint_ids]
            self._q_des[sync_mask] = q[sync_mask]
            self._dq_des[sync_mask] = 0.0
            self._tau_ff[sync_mask] = 0.0
            self._applied_delta[sync_mask] = 0.0
        self._prev_unlock_mask[:] = unlock_mask
        log = self._ensure_log()
        log["stage2_action/newly_unlocked_ratio"] = newly_unlocked.float().mean().detach()
        log["stage2_action/newly_unlocked_sync_ratio"] = sync_mask.float().mean().detach()
    ...
    def _apply_stage2_qerr_soft_guard(self, arm_delta: torch.Tensor) -> torch.Tensor:
        """Smoothly suppress only Stage-2 deltas that increase an existing q error."""
        log = self._ensure_log()
        unlock_mask = self._physical_door_unlocked()
        if not self.cfg.use_stage2_qerr_soft_guard or not torch.any(unlock_mask):
            log["stage2_qerr_guard/active_joint_ratio"] = torch.zeros((), device=self.device)
            log["stage2_qerr_guard/mean_scale"] = torch.ones((), device=self.device)
            return arm_delta

        q = self._asset.data.joint_pos[:, self._joint_ids]
        qerr = self._q_des - q
        abs_qerr = qerr.abs()
        worsening = (qerr * arm_delta) > 0.0
        scale = (
            (self._stage2_qerr_hard_limit - abs_qerr)
            / (self._stage2_qerr_hard_limit - self._stage2_qerr_soft_limit)
        ).clamp(0.0, 1.0)
        guard_scale = torch.where(worsening, scale, torch.ones_like(scale))
        guarded = torch.where(unlock_mask[:, None], arm_delta * guard_scale, arm_delta)
        active = unlock_mask[:, None] & worsening & (abs_qerr >= self._stage2_qerr_soft_limit)
        log["stage2_qerr_guard/active_joint_ratio"] = active.float().mean().detach()
        log["stage2_qerr_guard/mean_scale"] = guard_scale[unlock_mask].mean().detach()
        log["stage2_qerr_guard/qerr_abs_max"] = abs_qerr[unlock_mask].max().detach()
        return guarded

    def _enforce_stage2_qerr_emergency_limit(self) -> None:
        """Last-resort symmetric qerr bound; normally the soft guard prevents reaching it."""
        log = self._ensure_log()
        unlock_mask = self._physical_door_unlocked()
        if not self.cfg.use_stage2_qerr_soft_guard or not torch.any(unlock_mask):
            log["stage2_qerr_guard/emergency_joint_ratio"] = torch.zeros((), device=self.device)
            return
        q = self._asset.data.joint_pos[:, self._joint_ids]
        qerr = self._q_des - q
        limited_qerr = torch.clamp(qerr, -self._stage2_qerr_hard_limit, self._stage2_qerr_hard_limit)
        emergency = unlock_mask[:, None] & (qerr.abs() > self._stage2_qerr_hard_limit)
        self._q_des[unlock_mask] = q[unlock_mask] + limited_qerr[unlock_mask]
        log["stage2_qerr_guard/emergency_joint_ratio"] = emergency.float().mean().detach()
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        arm_delta = self._apply_stage2_qerr_soft_guard(arm_delta)
        previous_q_des = self._q_des.clone()
        ...
        self._enforce_stage2_qerr_emergency_limit()
        self._applied_delta[:] = self._q_des - previous_q_des
        self._log_delta_stats(raw_delta, self._applied_delta, self._physical_door_unlocked())
...
class HighLevelDoorOpenActionCfg(ActionTermCfg):
    ...
    # Keep the Stage-1 holding target continuous through unlock. Enabling this
    # resets q_des to the measured q and can abruptly remove hook contact force.
    sync_arm_target_on_unlock: bool = False
    use_stage2_tracking_error_limit: bool = True
    stage2_tracking_error_limit: tuple[float, ...] | float = (0.15, 0.15, 0.12, 0.10, 0.10, 0.10)
    use_stage2_qerr_soft_guard: bool = False
    stage2_qerr_soft_limit: tuple[float, ...] | float = 0.25
    stage2_qerr_hard_limit: tuple[float, ...] | float = 0.40
