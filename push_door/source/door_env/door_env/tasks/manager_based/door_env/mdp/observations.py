...
def door_panel_forward_axis_b_xy(env, robot_cfg=SceneEntityCfg("robot"), door_cfg=SceneEntityCfg("door"),
                                 panel_axis_d=(0.0, -1.0, 0.0), eps=1.0e-6):
    """Dynamic door-panel direction in robot base frame, suitable for deployment."""
    door: Articulation = env.scene[door_cfg.name]
    axis_d = _expand_vec3(panel_axis_d, door.data.root_pos_w)
    axis_w = quat_rotate(door.data.root_quat_w, axis_d)
    axis_b = _world_vector_to_base(env, axis_w, robot_cfg)[:, :2]
    return axis_b / torch.clamp(torch.linalg.norm(axis_b, dim=-1, keepdim=True), min=float(eps))


def joint_position_scalar(env, asset_cfg):
    asset: Articulation = env.scene[asset_cfg.name]
    return asset.data.joint_pos[:, asset_cfg.joint_ids[:1]]


def joint_velocity_scalar(env, asset_cfg):
    asset: Articulation = env.scene[asset_cfg.name]
    return asset.data.joint_vel[:, asset_cfg.joint_ids[:1]]


def door_open_direction(
    env: ManagerBasedRLEnv,
    door_open_sign: float = 1.0,
) -> torch.Tensor:
    """Return the task's signed door-opening direction as privileged state."""
    return torch.full(
        (env.num_envs, 1),
        float(door_open_sign),
        dtype=torch.float32,
        device=env.device,
    )


def pull_zone_one_hot(
    env: ManagerBasedRLEnv,
    robot_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    ...
    """Return frozen Pull traverse zone as Z1/Z2/Z3/outside one-hot."""
    from .rewards import compute_door_zone_masks

    robot: Articulation = env.scene[robot_cfg.name]
    point_xy = robot.data.root_pos_w[:, :2]
    common = dict(
    ...
    live = compute_door_zone_masks(env, point_xy, **common)
    started = getattr(env, "_pull_traverse_started", None)
    frozen_ready = all(
        hasattr(env, name)
        for name in ("_pull_traverse_frozen_hinge_xy", "_pull_traverse_frozen_d0", "_pull_traverse_frozen_d")
    )
    if started is not None and frozen_ready:
        frozen = compute_door_zone_masks(
            env,
            point_xy,
            **common,
        ...
        use_frozen = started.to(dtype=torch.bool)
        if hasattr(env, "episode_length_buf"):
            use_frozen = use_frozen & (env.episode_length_buf > 0)
        masks = {
            key: torch.where(use_frozen, frozen[key], live[key])
            for key in ("z1", "z2", "z3")
        }
    else:
        masks = live
    outside = ~(masks["z1"] | masks["z2"] | masks["z3"])
    return torch.stack((masks["z1"], masks["z2"], masks["z3"], outside), dim=-1).float()


def pull_traverse_state_one_hot(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Return Pull traverse history state 0/1/2/3 as one-hot."""
    state = getattr(env, "_pull_traverse_state", None)
    ...
    if hasattr(env, "episode_length_buf"):
        state = torch.where(env.episode_length_buf == 0, torch.zeros_like(state), state)
    return torch.nn.functional.one_hot(torch.clamp(state, 0, 3), num_classes=4).float()


def pull_traverse_started(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Return whether the frozen Pull traverse phase has started."""
    value = getattr(env, "_pull_traverse_started", None)
    if value is None or value.shape[0] != env.num_envs:
        value = torch.zeros(env.num_envs, dtype=torch.bool, device=env.device)
    if hasattr(env, "episode_length_buf"):
        value = value & (env.episode_length_buf > 0)
    return value.float().unsqueeze(-1)


def pull_traverse_cheat_given(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Return whether a direct Z3-to-Z2 shortcut occurred this episode."""
    value = getattr(env, "_pull_traverse_cheat_given", None)
    if value is None or value.shape[0] != env.num_envs:
        value = torch.zeros(env.num_envs, dtype=torch.bool, device=env.device)
    if hasattr(env, "episode_length_buf"):
        value = value & (env.episode_length_buf > 0)
...
def unlock_state(env):
    mode = getattr(env, "_door_lock_mode", None)
    if mode is not None:
        return (mode == 2).float().unsqueeze(-1)
    return torch.zeros((env.num_envs, 1), device=env.device)


def stage_id(env):
    grasped = getattr(env, "_grasp_success_given", torch.zeros(env.num_envs, device=env.device, dtype=torch.bool))
    unlocked = unlock_state(env).squeeze(-1).bool()
    stage = torch.where(grasped, torch.ones(env.num_envs, device=env.device, dtype=torch.long),
                        torch.zeros(env.num_envs, device=env.device, dtype=torch.long))
    return torch.where(unlocked, torch.full_like(stage, 2), stage).float().unsqueeze(-1)
