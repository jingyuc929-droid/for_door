...
def bad_arm_pose(
    ...
    """Terminate after traversal when the gripper center is below the base."""
    ...
    return (signed_forward > float(pass_distance)) & (
...
def base_fall(
    ...
    max_base_height: float | None = None,
) -> torch.Tensor:
    """Terminate immediately when the robot base height leaves its valid range."""
    robot: Articulation = env.scene[robot_cfg.name]
    base_height = robot.data.root_pos_w[:, 2]
    done = base_height < float(min_base_height)
    if max_base_height is not None:
        done = done | (base_height > float(max_base_height))
    if hasattr(env, "extras"):
        if env.extras is None:
            env.extras = {}
...
def base_out_of_hinge_radius(
    ...
    """Terminate when the base leaves the traverse disk around the hinge."""
    robot: Articulation = env.scene[robot_cfg.name]
    ...
    done = zones["radial_distance"] > float(radius)
    if hasattr(env, "extras"):
        if env.extras is None:
            env.extras = {}
        log = env.extras.setdefault("log", {})
        log["termination/base_hinge_radius_mean"] = zones["radial_distance"].mean().detach()
        log["termination/base_out_of_hinge_radius_ratio"] = done.float().mean().detach()
    return done
