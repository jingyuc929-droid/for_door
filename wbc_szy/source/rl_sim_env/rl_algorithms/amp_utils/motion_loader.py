...
class AMPLoader:
    def __init__(
        ...
        self._trajectory_duration_t = torch.as_tensor(
            self.trajectory_duration, device=device, dtype=torch.float32
        )
        self._trajectory_num_frames_t = torch.as_tensor(
            self.trajectory_num_frames, device=device, dtype=torch.long
        )
        ...
        # vstack owns the contiguous trajectory table from here on.  Keeping the
        # per-file GPU tensors would retain a complete second copy on every rank.
        self.trajectories_full.clear()

        # Preload transitions.
        print(f"Preloading {num_preload_transitions} transitions")
        traj_idxs = self.weighted_traj_idx_sample_batch(num_preload_transitions)
        times = self.traj_time_sample_batch(traj_idxs)
        traj_idxs_t = torch.as_tensor(
            traj_idxs, device=device, dtype=torch.long
        )
        times_t = torch.as_tensor(times, device=device, dtype=torch.float32)
        combined_indices_t = torch.as_tensor(
            self.combined_indices, device=device, dtype=torch.long
        )
        # PPO only samples the AMP subset.  Drop unused full-frame columns after
        # interpolation instead of keeping them for the entire training run.
        self.preloaded_s = self.get_full_frame_at_time_batch(
            traj_idxs_t, times_t
        ).index_select(1, combined_indices_t)
        self.preloaded_s_next = self.get_full_frame_at_time_batch(
            traj_idxs_t, times_t + self.time_between_frames
        ).index_select(1, combined_indices_t)
    ...
    def get_full_frame_at_time_batch(self, traj_idxs, times):
        ...
        n_frames = self._trajectory_num_frames_t[traj_idxs_t]  # [B]
        duration = self._trajectory_duration_t[traj_idxs_t]  # [B]
