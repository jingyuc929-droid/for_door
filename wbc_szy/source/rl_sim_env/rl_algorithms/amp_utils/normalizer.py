...
class Normalizer(RunningMeanStd):
    def __init__(self, input_dim, epsilon=1e-4, clip_obs=10.0):
        ...
        self._torch_stats_cache = {}

    def update(self, arr: np.ndarray) -> None:
        super().update(arr)
        # NumPy statistics changed; rebuild the device tensors lazily on the
        # next normalize_torch() call.
        self._torch_stats_cache.clear()
    ...
    def normalize_torch(self, input, device):
        ...
        cache = getattr(self, "_torch_stats_cache", None)
        if cache is None:
            cache = {}
            self._torch_stats_cache = cache
        stats = cache.get(device)
        if stats is None:
            ...
            stats = (mean_torch, std_torch)
            cache[device] = stats
        else:
            mean_torch, std_torch = stats
        return torch.clamp((input - mean_torch) / std_torch, -self.clip_obs, self.clip_obs)

    def __getstate__(self):
        state = self.__dict__.copy()
        # The cache is derived data and should not make checkpoints device-bound.
        state["_torch_stats_cache"] = {}
        return state
