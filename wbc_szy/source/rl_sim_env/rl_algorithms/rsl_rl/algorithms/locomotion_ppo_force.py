...
class LocomotionPPOForce:
    """LocomotionPPO + 末端外力显式 head 的 VAE MSE 监督（force_control 专属）。

    与 LocomotionPPO 的唯一区别：VAE loss 多一项 force MSE（code_force vs
    gt_force_obs），adaboot gt_out 多拼 gt_force_obs。code 拼接顺序 [vel,com,mass,force,latent]。
    """
    ...
    def act(self, obs):
        if self.use_vae:
            ...
            if self.vae_use_adaboot:
                if self.p_boot > 0.0:
                    gt_out = torch.cat((obs['gt_vel_obs'], obs['gt_com_obs'], obs['gt_mass_obs'], obs['gt_force_obs'], vae_out['code_latent']), dim=-1).detach()
    ...
    def update(self):  # noqa: C901
        ...
        for sample_step in data_generator:
            ...
            if self.use_vae:
                ...
                vae_force_target = obs_batch["gt_force_obs"].detach()
                ...
                loss_recon_force = nn.MSELoss()(vae_out["code_force"], vae_force_target)
                loss_recon_decode = nn.MSELoss()(vae_out["decoded"], vae_decode_target)
                loss_recon = loss_recon_vel + loss_recon_mass + loss_recon_com + loss_recon_force + loss_recon_decode
            ...
            if self.use_vae:
                ...
                mean_vae_force_loss += loss_recon_force.item()
        ...
        if self.use_vae:
            ...
            mean_vae_force_loss /= num_updates
            ...
            loss_debug_dict_vae = {
                ...
                "vae_force_loss": mean_vae_force_loss,
