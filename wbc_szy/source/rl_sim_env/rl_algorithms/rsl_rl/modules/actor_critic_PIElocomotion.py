...
class ActorCriticPIElocomotion(nn.Module):
    def __init__(
        self,
        module_cfg_dict
    ):
    ...
    def act(self, obs):
        ...
        if 'PIE_estimator_net_encoder_out' in obs:
            whole_actor_obs = torch.cat((obs['PIE_estimator_net_encoder_out'], actor_obs), dim=-1)
        else:
            whole_actor_obs = actor_obs
        self.update_distribution(whole_actor_obs)
    ...
    def act_inference(self, obs):
        ...
        if 'PIE_estimator_net_encoder_out' in obs:
            ...
            whole_actor_obs = actor_obs
        return self.actor(whole_actor_obs)

    def evaluate(self, obs):
        critic_obs = self.get_critic_obs(obs)
        critic_obs = self.critic_obs_normalizer(critic_obs)
        privileged_obs = self.get_privileged_obs(obs)
        obs = torch.cat((critic_obs, privileged_obs), dim=-1)
