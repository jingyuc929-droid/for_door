...
from rl_algorithms.rsl_rl.algorithms import PIElocomotionPPO
from rl_algorithms.rsl_rl.env import VecEnv
from rl_algorithms.rsl_rl.modules import ActorCriticPIElocomotion, AMPDiscriminator, PIEEstimatorNet
from rl_algorithms.rsl_rl.utils import store_code_state


class PIElocomotionOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cuda:0"):
        ...
        # check if use PIE estimator net
        self.use_PIE_estimator_net = self.train_cfg_dict["use_PIE_estimator_net"] if "use_PIE_estimator_net" in self.train_cfg_dict else False
        if self.use_PIE_estimator_net:
            self.use_PIE_estimator_net_exclusive_optimizer = self.train_cfg_dict["PIE_estimator_net"]["use_exclusive_optimizer"] if "use_exclusive_optimizer" in self.train_cfg_dict["PIE_estimator_net"] else False
        else:
        ...
        actor_critic: ActorCriticPIElocomotion = eval(self.cfg["policy_type"]["actor_critic_type"])(
        ...
        # PIE estimator net
        if self.use_PIE_estimator_net:
            PIE_estimator_net: PIEEstimatorNet = eval(self.cfg["policy_type"]["PIE_estimator_net_type"])(self.module_cfg_dict['PIE_estimator_net']).to(self.device)
            module_dict['PIE_estimator_net'] = PIE_estimator_net

        # initialize algorithm
        self.alg = PIElocomotionPPO(
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    # Sample actions
                    actions = self.alg.act(obs_dict.clone())
                    if self.use_amp:
                    ...
                    next_actor_obs = obs_dict["next_obs"].detach().clone()
                    # process the step
    ...
    def save(self, path: str, infos=None):
        ...
        if self.use_PIE_estimator_net   :
            saved_dict_PIE_estimator_net = {
                "PIE_estimator_net_state_dict": self.alg.PIE_estimator_net.state_dict(),
            }
            if self.use_PIE_estimator_net_exclusive_optimizer:
                saved_dict_PIE_estimator_net["PIE_estimator_net_optimizer_state_dict"] = self.alg.optimizer_dict['PIE_estimator_net'].state_dict()
            saved_dict.update(saved_dict_PIE_estimator_net)
    ...
    def load(self, path: str, load_optimizer: bool = True):
        ...
        if self.use_PIE_estimator_net:
            self.alg.PIE_estimator_net.load_state_dict(loaded_dict["PIE_estimator_net_state_dict"], strict=True)
    ...
    def get_inference_policy(self, device=None):
        ...
        if device is not None:
            ...
            if self.use_PIE_estimator_net:
                self.alg.PIE_estimator_net.to(device)
        policy = self.alg.actor_critic
        if self.use_PIE_estimator_net:
            extra_dict["PIE_estimator_net"] = self.alg.PIE_estimator_net
    ...
    def train_mode(self):
        ...
        # -- PIE estimator net
        if self.use_PIE_estimator_net:
            self.alg.PIE_estimator_net.train()
    ...
    def eval_mode(self):
        ...
        # -- PIE estimator net
        if self.use_PIE_estimator_net:
            self.alg.PIE_estimator_net.eval()
