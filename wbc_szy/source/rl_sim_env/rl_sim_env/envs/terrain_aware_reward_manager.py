...
class TerrainAwareRewardManager(RewardManager):
    ...
    def __init__(self, cfg: object, env: ManagerBasedRLEnv):
        ...
        # terrain_types 与配置权重在训练期间不变；setup 时一次性判定哪些 term
        # 在当前 env 集合里确实有非零权重，避免 compute() 每步把 CUDA all()
        # 转成 Python bool 所造成的 GPU->CPU 同步。
        self._active_terrain_reward_terms: set[str] = set()
    ...
    def _setup_terrain_weights(self):
        """扫描所有奖励项，设置地形相关的权重。"""
        self.terrain_reward_weights.clear()
        self._active_terrain_reward_terms.clear()
        ...
        for term_idx, (term_name, term_cfg) in enumerate(zip(self._term_names, self._term_cfgs)):
            ...
            if has_terrain_weights:
                ...
                if has_nonzero_weight:
                    self._active_terrain_reward_terms.add(term_name)
    ...
    def compute(self, dt: float) -> torch.Tensor:
        ...
        for term_idx, (name, term_cfg) in enumerate(zip(self._term_names, self._term_cfgs)):
            ...
            if name in self.terrain_reward_weights:
                # 使用地形相关权重（每个环境不同）
                # 静态判定已在 setup 完成；这里只查 Python set，不同步 CUDA。
                if name not in self._active_terrain_reward_terms:
