...
def _resolve_ee_tool_quat_offset(env, command_name: str) -> torch.Tensor:
    """读取 command cfg 上的 ee_tool_quat_offset（末端工具系相对 link6 系的旋转）。

    返回 (num_envs, 4) 的 (w, x, y, z) 四元数张量。缺省为单位四元数（即 link6
    局部 +x 就是工具指向，如 X5）。x/y offset 跟踪奖励在比较末端姿态前先用它把
    link6 系转到工具系，使“任务系 +x”在两臂上都恒等于物理工具指向。
    """
    default = (1.0, 0.0, 0.0, 0.0)
    q = default
    cmd_term = _get_command_term(env, command_name)
    if cmd_term is not None:
        cached_q = getattr(cmd_term, "_tool_quat_offset", None)
        if (
            isinstance(cached_q, torch.Tensor)
            and cached_q.shape == (env.num_envs, 4)
            and cached_q.device == torch.device(env.device)
        ):
            return cached_q
        cfg = getattr(cmd_term, "cfg", None)
        if cfg is not None and hasattr(cfg, "ee_tool_quat_offset"):
            try:
                val = tuple(float(c) for c in getattr(cfg, "ee_tool_quat_offset"))
                if len(val) == 4:
                    q = val
            except Exception:
                q = default
    return torch.tensor(q, device=env.device, dtype=torch.float32).repeat(
        env.num_envs, 1
    )


def _get_cached_projected_com_yaw_frame(
    env,
    command_name: str,
    robot: Articulation,
    trunk_body_id: int,
    ...
    """Per-step cached projected-COM yaw frame shared across EE rewards.

    所有 EE 奖励（main / x-offset / y-offset / pitch-height）在同一步内解析的
    是同一个参考系（同机器人、同 trunk、同命令），这里按 ``common_step_counter``
    缓存，避免每个 reward term 都重复计算 frame（尤其避免重复触发
    ``root_physx_view.get_coms()``）。地面高度 LPF 状态仍由命令侧写入，这里只读
    （``read_cached_ground_z_only=True``），因此缓存不影响命令侧的写入。
    """
    step = getattr(env, "common_step_counter", None)
    cache_key = (command_name, int(trunk_body_id))
    if step is not None:
        if (
            getattr(env, "_manip_ee_frame_step", None) == step
            and getattr(env, "_manip_ee_frame_key", None) == cache_key
        ):
            return getattr(env, "_manip_ee_frame")
    ...
    if step is not None:
        setattr(env, "_manip_ee_frame_step", step)
        setattr(env, "_manip_ee_frame_key", cache_key)
        setattr(env, "_manip_ee_frame", frame)
    return frame
...
def _get_ee_target_reference_frame(
    ...
    if use_projected_origin:
        ...
        return _get_cached_projected_com_yaw_frame(
            env, command_name, robot, trunk_body_id, terrain_sensor
        )
    else:
...
def _get_cached_command_active(
    env, command_name: str, command_active_threshold: float
) -> tuple[torch.Tensor, torch.Tensor]:
    """Return a command and its active mask, shared by reward terms this step."""
    cmd = env.command_manager.get_command(command_name)
    cache_key = (
        command_name,
        float(command_active_threshold),
        id(cmd),
        getattr(cmd, "_version", None),
    )
    step = getattr(env, "common_step_counter", None)
    if (
        step is not None
        and getattr(env, "_manip_command_active_step", None) == step
        and getattr(env, "_manip_command_active_key", None) == cache_key
    ):
        return cmd, getattr(env, "_manip_command_active")

    active = torch.linalg.norm(cmd, dim=1) > command_active_threshold
    if step is not None:
        setattr(env, "_manip_command_active_step", step)
        setattr(env, "_manip_command_active_key", cache_key)
        setattr(env, "_manip_command_active", active)
    return cmd, active


def _get_cached_ee_target_points_state(
    env,
    command_name: str,
    ...
    """Compute the EE main/x/y points once for the three split rewards."""
    cmd, cmd_active = _get_cached_command_active(
        env, command_name, command_active_threshold
    )
    cache_key = (
        command_name,
        id(robot),
    ...
    if (
        step is not None
        and getattr(env, "_manip_ee_points_step", None) == step
        and getattr(env, "_manip_ee_points_key", None) == cache_key
    ):
        return getattr(env, "_manip_ee_points_state")
    ...
    ee_quat_p = quat_mul(
        ee_quat_p, _resolve_ee_tool_quat_offset(env, command_name)
    )
    local_x = torch.zeros_like(ee_pos_p)
    local_y = torch.zeros_like(ee_pos_p)
    ...
    state = (cmd, cmd_active, ee_pos_p, ee_x_p, ee_y_p)
    if step is not None:
        setattr(env, "_manip_ee_points_step", step)
        setattr(env, "_manip_ee_points_key", cache_key)
        setattr(env, "_manip_ee_points_state", state)
    return state


def _clamp_force_compliance_delta_to_workspace(
    env,
    command_name: str,
    cmd_main: torch.Tensor,
    ...
    *,
    ...
    """Clamp a shared compliance delta once per policy step.

    The main and pitch rewards must consume exactly the same effective target.  Caching the
    already-clamped delta both enforces that invariant and avoids repeating the full-batch box
    reduction (and recreating its CUDA bounds tensors) for every reward term.
    """
    if not enabled or r_max is None or z_range is None:
        return delta_p

    r_lim = max(float(r_max), 0.0)
    ...
    if (
        step is not None
        and getattr(env, "_manip_force_workspace_step", None) == step
        and getattr(env, "_manip_force_workspace_key", None) == cache_key
    ):
        return getattr(env, "_manip_force_workspace_delta")

    bounds_cache = getattr(env, "_manip_force_workspace_bounds", None)
    if bounds_cache is None:
        bounds_cache = {}
        setattr(env, "_manip_force_workspace_bounds", bounds_cache)
    ...
    clamped_delta = scale_delta_p_to_box(cmd_main, delta_p, *bounds)
    if step is not None:
        setattr(env, "_manip_force_workspace_step", step)
        setattr(env, "_manip_force_workspace_key", cache_key)
        setattr(env, "_manip_force_workspace_delta", clamped_delta)
...
def track_ee_target_points_exp(
    ...
    # TCP 偏置：把 link6 系转到工具系（工具 +x = 物理工具指向）
    ee_quat_p = quat_mul(ee_quat_p, _resolve_ee_tool_quat_offset(env, command_name))
...
def track_ee_target_main_exp(
    ...
    ee_body_id = ee_asset_cfg.body_ids[0]
    trunk_body_id = trunk_asset_cfg.body_ids[0]
    command_term = _get_command_term(env, command_name)
    shared_offset_distance = float(
        getattr(command_term, "offset_distance", 0.0)
    )
    cmd, cmd_active, ee_pos_p, _, _ = _get_cached_ee_target_points_state(
        env,
        command_name,
...
def track_ee_target_x_offset_exp(
    ...
    ee_body_id = ee_asset_cfg.body_ids[0]
    trunk_body_id = trunk_asset_cfg.body_ids[0]
    cmd, cmd_active, _, ee_x_p, _ = _get_cached_ee_target_points_state(
        env,
        command_name,
        command_active_threshold,
        robot,
...
def track_ee_target_y_offset_exp(
    ...
    ee_body_id = ee_asset_cfg.body_ids[0]
    trunk_body_id = trunk_asset_cfg.body_ids[0]
    cmd, cmd_active, _, _, ee_y_p = _get_cached_ee_target_points_state(
        env,
        command_name,
        command_active_threshold,
        robot,
...
def track_ee_target_main_exp_force_compliance(
    ...
    if cfg_cmd is None:
        ...
        z_range = getattr(
            cfg_cmd,
            "ee_force_compliance_workspace_z_range",
            getattr(cfg_cmd, "ee_target_pos_z_range", (0.0, 0.0)),
        )
    if not enable:
        scale = (0.0, 0.0, 0.0)
    ...
    delta_p = _clamp_force_compliance_delta_to_workspace(
        env,
        command_name,
        cmd_main,
        delta_p,
...
def track_pitch_with_ee_target_height_exp_force_compliance(
    ...
    command_active_threshold: float = 1.0e-6,
    upright_scale_max: float | None = 0.7,
    trunk_asset_cfg: SceneEntityCfg = SceneEntityCfg(
    ...
    参考系通过与 main EE reward 相同的配置解析，避免把 robot/base_link/height_scanner
    硬编码进 pitch term 后，在切换 trunk-COM 或资产命名时悄悄产生不一致。
    """

    def _resolve_command_z_range(
        _env, _command_name: str, _fallback_z: torch.Tensor
    ) -> tuple[float, float]:
        cmd_term = _get_command_term(_env, _command_name)
    ...
    robot: Articulation = env.scene[trunk_asset_cfg.name]
    if len(trunk_asset_cfg.body_ids) != 1:
        raise ValueError(
            "trunk_asset_cfg.body_names 需包含 1 个躯干刚体，"
            f"实际为 {len(trunk_asset_cfg.body_ids)}"
