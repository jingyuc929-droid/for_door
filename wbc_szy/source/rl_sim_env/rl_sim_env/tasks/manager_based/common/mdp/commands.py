...
class UniformVelocityCommandTerrain(CommandTerm):
    ...
    def __init__(self, cfg: UniformVelocityCommandTerrainCfg, env: ManagerBasedEnv):
        ...
        # -- metrics
        self._split_xy_velocity_metrics = bool(
            getattr(cfg, "split_xy_velocity_metrics", False)
        )
        if self._split_xy_velocity_metrics:
            self.metrics["error_vel_x"] = torch.zeros(
                self.num_envs, device=self.device
            )
            self.metrics["error_vel_y"] = torch.zeros(
                self.num_envs, device=self.device
            )
        else:
    ...
    def _update_metrics(self):
        ...
        # logs data
        velocity_error_xy = torch.abs(
            self.vel_command_b[:, :2] - self.robot.data.root_lin_vel_b[:, :2]
        )
        if self._split_xy_velocity_metrics:
            self.metrics["error_vel_x"] += (
                velocity_error_xy[:, 0] / max_command_step
            )
            self.metrics["error_vel_y"] += (
                velocity_error_xy[:, 1] / max_command_step
            )
        else:
            self.metrics["error_vel_xy"] += (
                torch.norm(velocity_error_xy, dim=-1) / max_command_step
            )
