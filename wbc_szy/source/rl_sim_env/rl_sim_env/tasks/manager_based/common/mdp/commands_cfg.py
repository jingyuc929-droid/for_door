...
class UniformVelocityCommandTerrainCfg(CommandTermCfg):
    ...
    """速度课程每级的 episode 倍数（升级阈值 = (level+1) * max_episode_length * vel_curriculum_episode_mult）。piper_default=4，其余=8。"""

    split_xy_velocity_metrics: bool = False
    """Whether to log x/y velocity errors separately instead of one xy norm."""
