...
# ----------------------------
# Locomotion (2d4) rough + plane: 固定 80% random_rough / 20% plane
# ----------------------------
ROUGH_PLANE_TERRAINS_CFG2d4 = terrain_gen.TerrainGeneratorCfg(
    size=(10.0, 10.0),
    border_width=60.0,
    # 10 列可精确生成 8 列 rough 和 2 列 plane。
    num_rows=10,
    num_cols=10,
    horizontal_scale=0.1,
    vertical_scale=0.005,
    slope_threshold=0.75,
    use_cache=False,
    # 所有难度在训练开始时一次性生成，训练期间不做地形课程。
    curriculum=False,
    sub_terrains={
        "random_rough": terrain_gen.HfRandomUniformTerrainCfg(
            proportion=0.8, noise_range=(0.01, 0.06), noise_step=0.01, border_width=0.3
        ),
        "plane_run": terrain_gen.MeshPlaneTerrainCfg(
            proportion=0.2,
        ),
...
# grq20_v2d4 rough + plane：仅供需要固定 20% 平地、且不做地形课程的任务使用
LOCOMOTION_ROUGH_PLANE_TERRAIN_CFG2d4 = TerrainImporterCfg(
    prim_path="/World/ground",
    terrain_type="generator",
    terrain_generator=ROUGH_PLANE_TERRAINS_CFG2d4,
    # 初始即均匀分布到全部 level；任务侧会关闭 terrain level curriculum。
    max_init_terrain_level=None,
    collision_group=-1,
    physics_material=sim_utils.RigidBodyMaterialCfg(
        friction_combine_mode="multiply",
