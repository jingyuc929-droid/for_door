...
# 机械臂（piperL）关节：用于全身控制（腿+臂）。AMP/腿相关保持使用 ROBOT_JOINT_NAMES（仅腿12DOF）
...
# 末端工具系(TCP)偏置：link6 局部系 -> 工具系（工具 +x = 物理工具指向）。
# piper 的工具指向是 link6 局部 +z（joint6 绕局部 Z 转 = 腕部 roll），
# 零位下 link6 三轴为：+x≈朝下、+y≈侧向、+z≈朝前(工具)。
# 绕 y 轴 -90° 可把 +z 转到 +x（同时 +x 转到 +z、+y 不变），
# 四元数 (w,x,y,z) = (cos45, 0, -sin45, 0)。
# 该偏置让任务系（main/x/y 三点、姿态误差、x/y offset 跟踪）在 piper 上
# 与 X5 取一致的物理含义（工具 +x = 任务 +x = 前方）。
# 对比 X5：其 link6 +x 即工具指向，无需偏置（单位四元数）。
...
ROBOT_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        # grq20_v2d4_piperL: 四足 + piperL机械臂
        usd_path=(
    ...
    actuators={
        "base_legs": DelayedPDActuatorCfg(
            ...
            # effort_limit=120.0,
            # velocity_limit=20.0,
            effort_limit={".*_hip_joint": 150.0, ".*_thigh_joint": 150.0, ".*_calf_joint": 150.0},
            velocity_limit={".*_hip_joint": 20.0, ".*_thigh_joint": 20.0, ".*_calf_joint": 20.0},
            stiffness=75.0,
            damping=3,
            friction=0.05,
            armature=0.01,
...
class ConfigSummary:
    ...
    class env:
        ...
        # 仅 critic 使用的末端 9D 跟踪误差观测维度（当前 ee points - command）
        ee_target_points_error_dim = 9
        ...
        #              +num_actions(actions)+3(base_commands)+ee_target_points_dim+ee_target_points_error_dim
        ...
        num_next_obs = 12 + 2 * num_dof + num_actions + ee_target_points_dim
        num_critic_obs = num_next_obs + ee_target_points_error_dim
        ...
        # privileged_obs dims: push_vel(2) + mass(1) + com(3) + material(12) + actuator_gains(24) + lag(1) + joint_friction(18) + foot_clearance(4) = 65
            ...
            'ground_truth_obs': {
                ...
                'ee_target_points_error_gt': {
                    ...
                    'params': {
                        ...
                        # command_manager.compute() has just refreshed the same projected ground height.
                        # Reuse it here so critic-only error obs does not repeat the height-scanner argmin.
                ...
                'random_material_gt': {
                    'params': {
                        ...
                        # Material randomization is startup-only in this task.
                        "cache_static": True,
                    }
                },
                ...
                # 关节/电机摩擦系数（per-joint，由 randomize_joint_friction 写入 asset.data.joint_friction_coeff）
                'random_joint_friction_gt': {
                    'scale': 10.0,
                    # Joint-friction randomization is startup-only in this task.
                    'params': {
                        'asset_cfg': SceneEntityCfg("robot", joint_names=ROBOT_WHOLE_BODY_JOINT_NAMES),
                        'cache_static': True,
                    },
    ...
    class command:
        ...
        # Level 0 is stationary; levels 1..6 preserve the previous moving
        # schedule (0.5..1.0 m/s) instead of silently dropping one stage.
        max_lin_x_level: float = 6.0
        ang_z_level: float = 0.0
        max_ang_z_level: float = 6.0
        # Kept for legacy checkpoint and force-curriculum compatibility.  The
        # cuVAETest-local performance curriculum uses the explicit dwell below.
        vel_curriculum_episode_mult: float = 8.0

        # ----------------------------
        # Performance-driven base velocity curriculum (cuVAETest only)
        # ----------------------------
        # x and yaw advance independently.  y has two states: zero while x is
        # at level 0, then the full range for every later x level.
        lin_x_level_ranges = (
            (0.0, 0.0),
            (-0.25, 0.25),
            (-0.40, 0.40),
        ...
        # Direct episode MAE thresholds, indexed by the current level.  x's
        # gate requires both x and y to pass; yaw uses its own threshold only.
        # The last entry is useful for logging/config validation but cannot
        # trigger another upgrade.
        lin_x_mae_thresholds = (0.080, 0.100, 0.112, 0.124, 0.136, 0.148, 0.160)
        # y itself only has the stationary/active states, so its active-state
        # gate stays fixed instead of becoming looser as x keeps expanding.
        lin_y_mae_thresholds = (0.080, 0.120, 0.120, 0.120, 0.120, 0.120, 0.120)
        ang_z_mae_thresholds = (0.080, 0.100, 0.112, 0.124, 0.136, 0.148, 0.160)

        # 200 PPO iterations * 24 control steps per iteration.
        velocity_curriculum_min_level_steps = 4800
        # EMA weights are proportional to the number of complete episodes in
        # each asynchronous reset batch (128 episodes form one reference batch).
        velocity_curriculum_ema_alpha = 0.20
        velocity_curriculum_min_survival_rate = 0.90
        # With the current 1000-control-step episode, this means average
        # completed episode length must be strictly greater than 900 steps.
        velocity_curriculum_min_episode_length_ratio = 0.90
        velocity_curriculum_min_completed_episodes = 128
        # EMA already smooths several episode batches; one post-dwell pass keeps
        # the minimum residence close to the requested 200 PPO iterations.
        velocity_curriculum_required_successes = 1
        velocity_curriculum_evaluation_interval_steps = 24
        ...
        ee_target_pos_r_range = (0.30, 0.75)
        ee_target_pos_theta_range = (-1.3, 1.3)
        ee_target_pos_z_range = (0.1, 0.9)
        ...
        ranges = {
            ...
            "random_rough": UniformVelocityCommandTerrainCfg.Ranges(
                # The local curriculum materializes later levels in-place.  The
                # training task therefore starts with a true zero x/y/yaw command.
                lin_vel_x=(0.0, 0.0),
                lin_vel_y=(0.0, 0.0),
                ang_vel_z=(0.0, 0.0),
                heading=(-math.pi / 2, math.pi / 2),
...
class LocomotionWholeBodyPPORunnerCfg(LocomotionOnPolicyRunnerCfg):
    ...
    #save_interval = [0, 200, 500, 800, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 7000, 10000, 12000, 15000, 20000, 30000, 50000]
    save_interval = [0, 1500, 3000, 4000, 5000, 5500, 6000, 6500, 7000, 7200, 7400, 7600, 7800, 8000, 9000, 10000]
    experiment_name = "grq20_v2d4_piper_cuvaetest_locomotion"
    swanlab_project = "grq20_v2d4_piper_cuvaetest_locomotion"
