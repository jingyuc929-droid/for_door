...
from rl_sim_env.tasks.manager_based.locomotion.config.grq20_v2d4_piper_default.observations import (
    create_default_noise_and_delay_obs_cfg,
)
...
# 真机 link5_joint 的零点偏差约为 0.07 rad；其余关节保持原来的 0.02 rad。
ROBOT_JOINT_OFFSET_LIMITS = tuple(
    0.08 if joint_name == "link5_joint" else 0.02
    for joint_name in ROBOT_WHOLE_BODY_JOINT_NAMES
)
...
ROBOT_CFG = ArticulationCfg(
    ...
    actuators={
        ...
        # 机械臂六个关节共用一个延迟；与腿部 actuator 的延迟独立采样。
...
class ConfigSummary:
    ...
    class env:
        ...
        # privileged_obs dims:
        # push_vel(2) + mass(1) + com(3) + material(12) + actuator_gains(24)
        # + actuator_lag(2) + observation_lag(2) + motor_offset(18)
        # + arm_mass_ratio(6) + base_force(3) + ee_force(3)
        # + joint_friction(18) + foot_clearance(4) = 98
        num_privileged_obs = 98
        num_privileged_encoder_out = 32
        ...
        module_cfg_dict = {
            ...
            "actor_critic": {
                ...
                'privileged_encoder': {
                    ...
                    'num_privileged_encoder_out': num_privileged_encoder_out,
                },
                'heightmap_encoder': {
                ...
                "critic": {
                    'num_critic_obs': num_critic_obs + num_privileged_encoder_out + 32,
    ...
    class observation:
        obs_term_dict = {
            'ground_truth_obs': {
                ...
                # 六个机械臂刚体的相对质量变化，范围为 [-0.2, 0.2]。
                'random_arm_mass_gt': {
                    'scale': 1.0 / 0.2,
                    'params': {
                        "asset_cfg": SceneEntityCfg(
                            "robot", body_names=ROBOT_ARM_BODY_NAMES
                        ),
                        "cache_static": True,
                    },
                },
                # 两路实际施加力，均在 base_link 坐标系中表达。
                'base_external_force_gt': {
                    'scale': (1.0 / 40.0, 1.0 / 40.0, 1.0 / 60.0),
                    'params': {'channel_name': 'base'},
                },
                'ee_external_force_gt': {
                    'scale': (1.0 / 10.0, 1.0 / 10.0, 1.0 / 15.0),
                    'params': {'channel_name': 'ee'},
                },
                ...
                'observation_lag_gt': {},
                # reset_joint_offset 对 18 个电机独立采样，因此完整传入 18 维。
                'motor_offset_gt': {
                    'scale': tuple(1.0 / limit for limit in ROBOT_JOINT_OFFSET_LIMITS),
                },
            ...
            'noise_and_delay_obs': create_default_noise_and_delay_obs_cfg(
        ...
        policy_obs_dict = {
            ...
            'actor_obs': {
                'terms': [
                    'body_proprioception_nad',
                    'arm_proprioception_nad',
                    'actions_gt',
                    'base_commands_gt',
                    'ee_target_points_gt',
                ],
    ...
    class event:
        external_force_interval_range_s = (8.0, 10.0)
        external_force_ramp_duration_range_s = (0.0, 0.5)
        ...
        config_dict = {
            ...
            'reset_joint_offset': {
                ...
                'params': {
                    'randomization_params': (
                        tuple(-limit for limit in ROBOT_JOINT_OFFSET_LIMITS),
                        ROBOT_JOINT_OFFSET_LIMITS,
                    ),
                    'operation': "add",
            ...
            # reset 时立即采样首个目标，随后由同名 interval 通道独立重采样。
            'reset_base_external_force': {
                'mode': "reset",
                'params': {
                    'channel_name': "base",
                    'asset_cfg': SceneEntityCfg("robot", body_names=ROBOT_BASE_LINK),
                    'reference_body_cfg': SceneEntityCfg(
                        "robot", body_names=ROBOT_BASE_LINK
                    ),
                    'force_frame': "base",
                    'force_x_range': (-40.0, 40.0),
                    'force_y_range': (-40.0, 40.0),
                    'force_z_range': (-60.0, 60.0),
                    'ramp_duration_range_s': external_force_ramp_duration_range_s,
                },
            },
            'reset_ee_external_force_channel': {
                'mode': "reset",
                'params': {
                    'channel_name': "ee",
                    'asset_cfg': SceneEntityCfg(
                        "robot", body_names=ROBOT_EE_BODY_NAME
                    ),
                    'reference_body_cfg': SceneEntityCfg(
                        "robot", body_names=ROBOT_BASE_LINK
                    ),
                    'force_frame': "base",
                    'force_x_range': (-10.0, 10.0),
                    'force_y_range': (-10.0, 10.0),
                    'force_z_range': (-15.0, 15.0),
                    'ramp_duration_range_s': external_force_ramp_duration_range_s,
                },
            },
            'interval_base_external_force': {
                'mode': "interval",
                'interval_range_s': external_force_interval_range_s,
                'params': {
                    'channel_name': "base",
                    'asset_cfg': SceneEntityCfg("robot", body_names=ROBOT_BASE_LINK),
                    'reference_body_cfg': SceneEntityCfg(
    ...
    class command:
        ...
        # rough 与固定 20% 的 plane 共用此处的同一套参数，二者仅地形几何不同。
        # ----------------------------
        ...
        ee_target_pos_r_range = (0.3, 0.8)
        ee_target_pos_theta_range = (-1.3, 1.3)
        ee_target_pos_z_range = (0.1, 1.1)
        ee_target_roll_range = (-1.57, 1.57)
        ee_target_pitch_range = (-1.57, 1.57)
        ee_target_yaw_range = (-0.5, 0.5)

        # default 任务只有 rough / plane 两类地形。二者引用同一个对象，保证所有命令参数
        #（包括速度范围、模式概率和速度课程上下限）始终完全一致。
        shared_terrain_range = UniformVelocityCommandTerrainCfg.Ranges(
            lin_vel_x=(-0.5, 0.5),
            lin_vel_y=(-0.5, 0.5),
            ang_vel_z=(-0.25, 0.25),
        ...
        ranges = {
            "random_rough": shared_terrain_range,
            "plane_run": shared_terrain_range,
        }
...
class LocomotionWholeBodyPPORunnerCfg(LocomotionOnPolicyRunnerCfg):
    ...
    save_interval = 500
    experiment_name = "grq20_v2d4_piper_default_locomotion"
    swanlab_project = "grq20_v2d4_piper_default_locomotion"
