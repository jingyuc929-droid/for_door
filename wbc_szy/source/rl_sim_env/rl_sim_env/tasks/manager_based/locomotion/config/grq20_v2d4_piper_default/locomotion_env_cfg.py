...
from rl_sim_env.tasks.manager_based.common.terrain.config import (
    LOCOMOTION_ROUGH_PLANE_TERRAIN_CFG2d4,
)
...
class LocomotionWholeBodyVaeEnvCfg(LocomotionEnvCfg):
    def __post_init__(self):
        ...
        # terrain settings:
        # 固定 80% random_rough + 20% plane；两类地形的奖励与命令参数完全一致。
        # 不启用地形课程，env 在整个 tile 网格上固定分布。
        # 具体生成器参数集中在 common/terrain/config.py
        self.scene.terrain = LOCOMOTION_ROUGH_PLANE_TERRAIN_CFG2d4
        ...
        # observations
        # These terms are intentionally injected only for the Piper default task,
        # keeping the shared locomotion observation schema unchanged.
        ground_truth_obs = self.observations.ground_truth_obs
        ground_truth_obs.random_actuator_lag_gt = ObsTerm(func=actuator_lag)
        ground_truth_obs.observation_lag_gt = ObsTerm(func=observation_lag)
        ground_truth_obs.random_arm_mass_gt = ObsTerm(func=relative_body_mass)
        ground_truth_obs.base_external_force_gt = ObsTerm(
            func=external_force_channel
        )
        ground_truth_obs.ee_external_force_gt = ObsTerm(
            func=external_force_channel
        )
        ground_truth_obs.motor_offset_gt = ObsTerm(
            func=mdp.motor_offset,
            params={
                "asset_cfg": SceneEntityCfg(
                    "robot", joint_names=ROBOT_WHOLE_BODY_JOINT_NAMES
                ),
            },
        )
        noise_and_delay_obs = self.observations.noise_and_delay_obs
        noise_and_delay_obs.body_proprioception_nad = ObsTerm(
            func=body_proprioception
        )
        noise_and_delay_obs.arm_proprioception_nad = ObsTerm(
            func=arm_proprioception
        )
        ...
        # Default-only event slots. The force implementation is shared, while
        # these concrete channels and mass ranges remain local to this task.
        event_funcs = {
            "randomize_arm_mass": mdp.randomize_rigid_body_mass,
            "reset_base_external_force": mdp.start_external_force_channel_3d,
            "reset_ee_external_force_channel": mdp.start_external_force_channel_3d,
            "interval_base_external_force": mdp.start_external_force_channel_3d,
            "interval_ee_external_force": mdp.start_external_force_channel_3d,
        }
        for event_name, event_func in event_funcs.items():
            event_spec = self.config_summary.event.config_dict[event_name]
            event_kwargs = {
                "func": event_func,
                "mode": event_spec["mode"],
                "params": dict(event_spec["params"]),
            }
            if "interval_range_s" in event_spec:
                event_kwargs["interval_range_s"] = event_spec["interval_range_s"]
            self.events.__dict__[event_name] = EventTerm(**event_kwargs)
