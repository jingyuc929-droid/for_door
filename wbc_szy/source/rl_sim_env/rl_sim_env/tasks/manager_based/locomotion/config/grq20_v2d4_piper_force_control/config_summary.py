...
from rl_sim_env.tasks.manager_based.common.sensor.piper_observation_noise import (
    create_piper_noise_and_delay_obs_cfg,
)
...
# 机械臂（piper）关节：用于全身控制（腿+臂）。AMP/腿相关保持使用 ROBOT_JOINT_NAMES（仅腿12DOF）
...
# 末端工具系(TCP)偏置 (w,x,y,z)：piper link6 零位 +z≈朝前(工具)，绕 y -90° 把 +z 转到 +x，
# 让任务系 +x = 工具前方。reward/command 通过该偏置解耦 link6 帧约定。
ROBOT_EE_TOOL_QUAT_OFFSET = (0.70710678, 0.0, -0.70710678, 0.0)
...
ROBOT_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        # grq20_v2d4_piperL: 四足 + piperL机械臂 + 夹爪
        usd_path=(
            f"{RL_SIM_ENV_ASSETS_DIR}/robots/grq20_v2d5_piperL_fixedgrasper/"
            "grq20_v2d5_piperL_fixedgrasper.usd"
        ),
...
class ConfigSummary:
    ...
    class env:
        ...
        num_estimator_out = 26
        num_next_obs = num_critic_obs

        # privileged_obs dims:
        # push_vel(2) + mass(1) + com(3) + material(12) + actuator_gains(24) + lag(1) + foot_clearance(4)
        # + ee_external_force(3) + joint_friction(18) = 68
        ...
        policy_type = {
            ...
            'vae_type': "VAEBlindForce",
            'algo_type': "LocomotionPPOForce",
        }
        ...
        module_cfg_dict = {
            ...
            "vae": {
                ...
                'encoder_out_dim': 64,
                'encoder_head_dim_dict': {'obs_vel': 3, 'obs_com': 3, 'obs_mass': 1, 'obs_force': 3, 'obs_latent': 16},
    ...
    class observation:
        obs_term_dict = {
            'ground_truth_obs': {
                ...
                'push_vel_gt': {},
                # end-effector external force in world axes (3D)
                'ee_external_force_gt': {'scale': (0.1, 0.1, 0.1), },
                'random_material_gt': {
                    'params': {
                ...
                'random_joint_friction_gt': {'scale': 10.0,
                                             'params': {
                                                 "asset_cfg": SceneEntityCfg("robot", joint_names=ROBOT_WHOLE_BODY_JOINT_NAMES),
                                             }},
            ...
            'noise_and_delay_obs': create_piper_noise_and_delay_obs_cfg(
                ROBOT_JOINT_NAMES, ROBOT_ARM_JOINT_NAMES
            ),
            'amp_obs': {
        ...
        policy_obs_dict = {
            ...
            'privileged_obs': {
                'terms': ['push_vel_gt',
                          'ee_external_force_gt',
                          'random_mass_gt',
                          'random_com_gt',
                          'random_material_gt',
                          'random_actuator_gains_gt',
                          'random_actuator_lag_gt',
                          'random_joint_friction_gt',
                          'foot_clearance_gt',
                          ],
            },
            'gt_heightmap_obs': {
    ...
    class event:
        ...
        # ----------------------------
        # 说明：
        # - 该外力通过 EventTerm `ee_external_force` 写入仿真外力 buffer；
        # - 力/力矩均在 **世界坐标系** 下采样（x/y/z 轴分别独立采样）。
        # - 若希望固定外力，可将范围设为 (f, f)。
        # ----------------------------
        # 末端外力课程（仿照速度课程）
        # ----------------------------
        # 说明：
        # - level 从 0 逐级提升到 max_level；
        # - 每隔 episode_mult 个 episode 的步数提升 1 级；
        # - 施力 range 按 level/max_level 从 start 线性插值到 max。
        ee_force_curriculum_enable = True
        ee_force_curriculum_max_level = 5.0
        ee_force_curriculum_episode_mult = 4.0

        # start/max ranges（世界坐标轴）
        ee_force_x_range_start = (0.0, 0.0)
        ee_force_x_range_max = (-30.0, 30.0)
        ee_force_y_range_start = (0.0, 0.0)
        ee_force_y_range_max = (-30.0, 30.0)
        ee_force_z_range_start = (0.0, 0.0)
        ee_force_z_range_max = (-30.0, 30.0)

        # torque ranges（世界坐标轴）：课程接口需要，默认不施加扭矩
        ee_torque_x_range_start = (0.0, 0.0)
        ee_torque_x_range_max = (0.0, 0.0)
        ee_torque_y_range_start = (0.0, 0.0)
        ee_torque_y_range_max = (0.0, 0.0)
        ee_torque_z_range_start = (0.0, 0.0)
        ee_torque_z_range_max = (0.0, 0.0)


        # 触发间隔（秒）：per-env 6-8s 随机重采样新外力目标
        ee_force_interval_range_s = (6.0, 8.0)
        ...
        config_dict = {
            ...
            'ee_external_force': {
                'mode': "interval",
                'interval_range_s': ee_force_interval_range_s,
                'params': {
                    # 指定施力 link：机械臂末端
                    'asset_cfg': SceneEntityCfg("robot", body_names=ROBOT_EE_BODY_NAME),
                    # 初始使用 start range，后续由课程项 ee_external_force_threshold 更新
                    'force_x_range': ee_force_x_range_start,
                    'force_y_range': ee_force_y_range_start,
                    'force_z_range': ee_force_z_range_start,
                    # 1s 线性 ramp 到新采样目标（None=阶跃 step-and-hold）
                    'ramp_duration_s': 1.0,
                }
            },
        }
    ...
    class reward:   
        ...
        config_dict = {
            ...
            # 机械臂末端位置跟随（投影COM yaw frame，6D 命令 main）+ 力偏移
            # - delta_p = (target_force - external_force) * scale（阻抗），由 force_compliance 统一计算
            # - 偏移管理器参数在 ConfigSummary.command 中配置（ee_force_compliance_*）
            'track_ee_target_main_exp_force_compliance': {
                'weight': 1.0,
                'params': {
            ...
            # 末端目标高度协同机身俯仰（外力顺应版）：
            # - 对应 vaetest 的 track_pitch_with_ee_target_height_exp，但用来映射
            #   z->pitch 的末端目标高度是“力顺应之后”的高度（cmd_z + delta_p[:, 2]，
            #   delta_p 由 force_compliance 统一按 step 缓存），与上面 main 奖励偏移后
            #   的目标一致。注意 actor 仍接收 raw main + target_force；这里定义的是
            #   两个奖励共同使用的 effective target，并非单独下发的位置命令 buffer。
            # - frame 同样复用 EE 奖励的 per-step 缓存，不重复计算。
            # - raw command 仍从 z=(0.1,0.85) 采样；force compliance 后的 effective
            #   target 允许扩展到独立工作空间 z=(0.1,1.1)。
            # - pitch 映射显式使用同一个 (0.1,1.1) 范围，使中性区间 (0.5,0.9)
            #   的低侧和高侧都各有 0.4m 的完整引导区。
            'track_pitch_with_ee_target_height_exp_force_compliance': {
                'weight': 0.4,
                'params': {
                    'command_name': 'ee_target_points',
                    'z_low': 0.10,
    ...
    class command:
        ...
        vel_curriculum_episode_mult: float = 4.0
        ...
        # ee target command (6D: main 位置 + target_force) in projected COM yaw frame
        ee_target_resampling_time_range = (12, 12)
        # ee target 位置 main 的 smooth ramp time (seconds)
        ...
        ee_target_debug_vis = False
        # debug marker size (meters)
        ee_target_marker_radius = 0.03
        ee_pos_marker_radius = 0.02
        ee_target_pos_r_range = (0.30, 0.60)
        ee_target_pos_theta_range = (-1.3, 1.3)
        ee_target_pos_z_range = (0.10, 0.85)
        # target_force 独立 interval（per-env 随机）+ 线性 ramp；幅值跟外力课程同步
        ee_target_force_interval_range_s = (6.0, 8.0)
        ee_target_force_ramp_time_s = 1.0

        # ----------------------------
        # 外力偏移管理器（force compliance）参数：供奖励侧统一读取
        # 参考系：Projected COM yaw frame（与 ee_target_points 命令/奖励一致）
        # ----------------------------
        # 必须显式启用；None 经 bool(None) 会变成 False，使所有 compliance reward
        # 退化为原始位置目标。
        ee_force_compliance_enable: bool = True
        # 是否对偏移做工作空间保护（保守盒子约束，避免目标点明显不可达）
        ee_force_compliance_workspace_clamp_enable: bool = True
        # compliance 后 effective target 的独立 z 工作空间；不改变 raw command
        # 的 ee_target_pos_z_range=(0.10,0.85)。
        ee_force_compliance_workspace_z_range = (0.10, 1.10)
        # 力死区（N）：小于该阈值的外力分量置 0（按轴）
        ee_force_compliance_force_deadzone = (3.0, 3.0, 3.0)
        # 外力 -> 位置偏移比例（m/N）
        ee_force_compliance_scale = (0.005, 0.005, 0.01)
        # 外力 clip（N）：逐轴限制，None 表示不 clip
        ee_force_compliance_force_clip = None
        # 输出偏移 clip（m）：逐轴限制，None 表示不 clip
        ee_force_compliance_delta_clip = None
...
@configclass
class LocomotionPPORunnerCfg(LocomotionOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    num_steps_per_env = 24
    max_iterations = 50000
    save_interval = 200
    # 与目录命名对齐（force_control）
    experiment_name = "grq20_v2d4_piper_force_control_locomotion"
    swanlab_project = "grq20_v2d4_piper_force_control_locomotion"
