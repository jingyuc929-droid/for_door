...
from rl_sim_env.tasks.manager_based.common.manipulation.command.config import (
    create_projected_com_target_force_command_cfg,
)
...
class LocomotionVaeEnvCfg(LocomotionEnvCfg):
    def __post_init__(self):
        ...
        # foot_clearance only consumes the center ray, so force_control uses a single-ray
        # foot scanner instead of the default 3x3 grid.
        foot_clearance_scanner_cfg = FOOT_SCANNER_CFG.copy()
        foot_clearance_scanner_cfg.pattern_cfg = patterns.GridPatternCfg(
            resolution=0.05,
            size=[0.0, 0.0],
        )
        self.scene.fl_foot_scanner = foot_clearance_scanner_cfg.copy()
        ...
        self.scene.fr_foot_scanner = foot_clearance_scanner_cfg.copy()
        ...
        self.scene.rl_foot_scanner = foot_clearance_scanner_cfg.copy()
        ...
        self.scene.rr_foot_scanner = foot_clearance_scanner_cfg.copy()
        ...
        # Only track bodies that are consumed by rewards/terminations. Tracking every robot body
        # adds contact-history work for the arm without changing force_control signals.
        contact_body_expr = "|".join(
            [ROBOT_BASE_LINK] + ROBOT_FOOT_NAMES + ROBOT_THIGH_NAMES + ROBOT_CALF_NAMES
        )
        self.scene.contact_forces = ContactSensorCfg(
            prim_path=f"{{ENV_REGEX_NS}}/Robot/({contact_body_expr})",
            history_length=3,
            track_air_time=True,
        )
        ...
        self.commands.ee_target_points = create_projected_com_target_force_command_cfg(
            asset_name="robot",
            ...
            force_interval_range_s=self.config_summary.command.ee_target_force_interval_range_s,
            force_ramp_time_s=self.config_summary.command.ee_target_force_ramp_time_s,
        )
