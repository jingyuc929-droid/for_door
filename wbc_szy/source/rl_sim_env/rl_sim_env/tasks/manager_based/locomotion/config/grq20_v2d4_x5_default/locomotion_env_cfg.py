...
class LocomotionWholeBodyVaeEnvCfg_PLAY(LocomotionWholeBodyVaeEnvCfg):
    def __post_init__(self):
        ...
        # ee 末端命令：play 时手动指定一组固定目标（不再随机采样），便于复现/调试。
        # 处理方式与上面 base_command 一致：用固定范围重建 ee_target_points。
        # 所有值均在 projected COM yaw 参考系下：
        #   - pos_r/pos_theta/pos_z 用极坐标定义 main 点（半径 m / 方位角 rad / 高度 m）；
        #   - roll/pitch/yaw 定义末端 link6 局部坐标系相对该投影系的姿态 (rad)。
        # 想完全固定目标：把每个范围设成 (v, v)（如下默认值）。
        # 想在某窄区间内随机：给一个小区间，如 pos_r_range=(0.45, 0.55)。
        self.commands.ee_target_points = create_projected_com_target_points_command_cfg(
            asset_name="robot",
            trunk_body_name=ROBOT_BASE_LINK,
            terrain_sensor_name="height_scanner",
            ...
            pos_r_range=(0.5, 0.5),       # main 点半径 (m)
            pos_theta_range=(0.0, 0.0),   # main 点方位角 (rad)，0 = 正前方
            pos_z_range=(0.6, 0.6),       # main 点高度 (m)
            # ---- 手动指定的末端姿态 (rad) ----
            roll_range=(-0, 0),
            pitch_range=(-0, 0),
            yaw_range=(-0, 0),
