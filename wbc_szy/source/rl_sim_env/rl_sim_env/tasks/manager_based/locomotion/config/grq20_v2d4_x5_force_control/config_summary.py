...
class ConfigSummary:
    ...
    class observation:
        obs_term_dict = {
            'ground_truth_obs': {
                ...
                'random_joint_friction_gt': {'scale': 1.0},
    ...
    class event:
        ...
        ee_force_curriculum_episode_mult = 8.0

        # start/max ranges（世界坐标轴）
        ee_force_x_range_start = (0.0, 0.0)
        ee_force_x_range_max = (-40.0, 40.0)
        ee_force_y_range_start = (0.0, 0.0)
        ee_force_y_range_max = (-40.0, 40.0)
        ee_force_z_range_start = (0.0, 0.0)
        ee_force_z_range_max = (-40.0, 40.0)
    ...
    class command:
        ...
        # ----------------------------
        ee_force_compliance_enable: bool = None
        # 是否对偏移做工作空间保护（保守盒子约束，避免目标点明显不可达）
        ee_force_compliance_workspace_clamp_enable: bool = None
        # 力死区（N）：小于该阈值的外力分量置 0（按轴）
        ee_force_compliance_force_deadzone = (3.0, 3.0, 3.0)
        # 外力 -> 位置偏移比例（m/N）
        ee_force_compliance_scale = (0.02, 0.02, 0.04)
...
class LocomotionPPORunnerCfg(LocomotionOnPolicyRunnerCfg):
    ...
    experiment_name = "grq20_v2d4_x5_force_control_locomotion"
    swanlab_project = "grq20_v2d4_x5_force_control_locomotion"
